...
def load_and_preprocess_data(train_path):
    ...
    features = df[['d0', 'd1', 'd2', 'i0', 'i1']]
    targets = df[['f0', 'f1']]
    
    scaler = StandardScaler()
    features_scaled = scaler.fit_transform(features)
    
    df_scaled = pd.DataFrame(features_scaled, columns=['d0', 'd1', 'd2', 'i0', 'i1'])
    df_scaled['county'] = df['County']  # Ensure this matches the actual column name
    df_scaled['year'] = df['Year']
    df_scaled[['f0', 'f1']] = targets
...
class LinearMixedEffectsModel(tf.keras.Model):
    def __init__(self, num_counties):
        ...
        # Fixed effects
        self.beta_loc = self.add_weight(shape=(5, 2), initializer='zeros', trainable=True, name='beta_loc')
        self.beta_scale = self.add_weight(shape=(5, 2), initializer='ones', trainable=True, name='beta_scale')
    ...
    def call(self, inputs):
        if isinstance(inputs, dict):
            d0, d1, d2, i0, i1, county = inputs['d0'], inputs['d1'], inputs['d2'], inputs['i0'], inputs['i1'], inputs['county']
        else:
            d0, d1, d2, i0, i1, county = inputs
        ...
        # Fixed effects
        X = tf.stack([d0, d1, d2, i0, i1], axis=1)
...
def train_model(train_path):
    ...
    features = (df_scaled['d0'].values, df_scaled['d1'].values, df_scaled['d2'].values, 
                df_scaled['i0'].values, df_scaled['i1'].values, county_indices)
    targets = df_scaled[['f0', 'f1']].values
...
def predict_and_save(model, scaler, test_path):
    ...
    test_features = test_df[['d0', 'd1', 'd2', 'i0', 'i1']]
    test_features_scaled = scaler.transform(test_features)  # [num_test_samples, 5]
    ...
    test_inputs = {
        ...
        'i0': test_features_scaled[:, 3],
        'i1': test_features_scaled[:, 4],
        'county': test_county_indices
    }

    # Make predictions
    predictions = model(test_inputs)  # [num_test_samples, 2]
    f0_predictions, f1_predictions = predictions[:, 0], predictions[:, 1]

    test_df['f0'] = f0_predictions.numpy()
    test_df['f1'] = f1_predictions.numpy()
