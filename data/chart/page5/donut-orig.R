...
create_donut_chart <- function(data, title, colors) {
  ...
  ggplot(data, aes(ymax = ymax, ymin = ymin, xmax = 4, xmin = 2, fill = Category)) +
    ...
    theme(
      ...
      plot.margin = margin(t = 55, r = 0, b = 0, l = 0) # Adjust plot margins if needed
    ) +
...
# Arrange the plots with a single legend at the bottom
grid.arrange(
  arrangeGrob(new_voters_chart, added_voters_chart, 
              removed_voters_chart, ncol = 3, 
              padding = unit(1, "line")  
  ),
  legend,
  ncol = 1,
  heights = c(8, 1, 2) # Adjust heights to bring the legend closer
)
